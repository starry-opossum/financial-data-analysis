...
核心流程
SQL 提取资产台账：从 ERP 数据库 erp_fd_asset 表获取资产期间、类别、原始成本、累计折旧、减值、账面净值等字段。

数据筛选：仅保留目标期间（如 2023‑03）及特定类别（“计算机及电子设备”）。

计算折旧率：折旧率 = 累计折旧 / 原始成本，识别折旧率 > 70% 的待出售资产。
...
1. **结构完全复用**：保留了原 README 的章节风格（项目概述、项目结构、每个模块的业务背景+核心流程+代码示例+运行方式）。
2. **突出可扩展性**：在项目结构中用 `(未来新增模块)` 占位，在“扩展计划”中列出后续打算添加的模块。
3. **与实际仓库一致**：删除了不存在的“人力成本分摊”内容，替换为当前仓库实际存在的“融资需求测算”和“固定资产”。数据库密码已提醒使用环境变量。
4. **格式友好**：使用代码块、表格、目录树，方便阅读和展示。

你可以将这段内容复制到仓库根目录的 `README.md` 中，覆盖原文件，然后提交并推送。如果需要调整某个模块的描述细节（比如更详细的代码示例或删减），请告诉我。
